...
for cancer_type in 'BLCA' 'COAD' 'HNSC' 'OV' 'UCEC'
do
	#cancer_type='BLCA'
	echo $cancer_type
	pyNBS_script='/cellar/users/t1jia/pyNBS/Example_Command_Line_Tools/run_pyNBS.py'
	network_file='/cellar/users/t1jia/pyNBS/data/Network_Files/CancerSubnetwork.txt'
	mutation_file='/cellar/users/t1jia/Data/Cancer_Mutations/'$cancer_type'_sm_data.txt'

	network_file_basename=${network_file##*/}
	network_name=${network_file_basename%.*}

	# K=3 for 'COAD', K=4 for 'BLCA', 'HNSC', 'OV', 'UCEC'
	K=4
	if [ "$cancer_type" == "COAD" ]
		then
		K=3
	...
	result_dir='/cellar/users/t1jia/pyNBS/Example_Command_Line_Tools/results/'$cancer_type'/'
	mkdir -p $result_dir
